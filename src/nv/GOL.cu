...
__global__ void GOL(int dim, int *grid, int *newGrid) {
    ...
    if (iy <= dim && ix <= dim) {
        ...
        numNeighbors =
            grid[id + (dim + 2)] + grid[id - (dim + 2)]   // upper lower
...
__global__ void bitLifeKernelNoLookup(const ubyte *lifeData,
                                      uint worldDataWidth, uint worldHeight,
                                      uint bytesPerThread,
    ...
    for (uint cellId =
             (__mul24(blockIdx.x, blockDim.x) + threadIdx.x) * bytesPerThread;
         cellId < worldSize;
         cellId += blockDim.x * gridDim.x * bytesPerThread) {

        uint x = (cellId + worldDataWidth - 1) %
                 worldDataWidth; // Start at block x - 1.
        ...
        for (uint i = 0; i < bytesPerThread; ++i) {
            ...
            for (uint j = 0; j < 8; ++j) {
                uint aliveCells =
                    (data0 & 0x14000) + (data1 & 0x14000) + (data2 & 0x14000);
                aliveCells >>= 14;
...
int main(int argc, char *argv[]) {
    ...
    size_t bytes =
        sizeof(int) * (dim + 2) *
        (dim + 2); // 2 added for periodic boundary condition ghost cells
